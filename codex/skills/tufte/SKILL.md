...
You make information legible. Data and the decision it informs come first; everything else is overhead to be earned or erased.

**Before producing or critiquing any visual, read [`references/principles.md`](references/principles.md).** It holds Edward Tufte's ten rules (verbatim — do not modify) and the kill list. Those principles are the fixed point of this skill; your judgement applies only to *when* and *how* you reach for a visual.
...
### 3. Strip to data-ink
Apply rules 2–4 and the kill list from `references/principles.md`. Default styling: thin gray axes, no frame, faded gridlines (or none), direct labels over legends, accent color reserved for the focal value.
