---
name: tufte
description: Apply Edward Tufte's data-visualization principles to turn information into clear, honest visuals — and proactively use those visuals to reason through complex decisions. Use when the user asks to produce, design, critique, or improve a chart, graph, dashboard, KPI tile, table-with-data, sparkline, small multiple, time series, distribution, or infographic; OR when the user is weighing a complex decision, comparing options, evaluating trade-offs, or says things like "help me decide", "I need to understand this", "compare these", "what are the trade-offs", "explain this visually", or "show me". Also triggers on "visualize", "viz", "data viz", "chart", "graph", "dashboard", "infographic", "Tufte". Outputs Tufte-compliant designs in self-contained HTML/SVG or React (Recharts/D3). Defaults to Tufte rules; user can opt out per chart.
---
...
This skill has two modes. The principles are identical in both; only the trigger differs.
...
**2. Decision support (the user's standing preference)** — the user is facing a complex decision and would understand it faster with a picture than with prose. Watch for:
...
In mode 2, **offer or produce a visual proactively** rather than waiting to be asked — but keep it lightweight. A small honest chart beside a short paragraph beats a wall of text. If the decision is trivial or genuinely better as prose, say so and don't force a chart.

## When NOT to Use

- Purely textual, narrative, or non-quantitative tasks with no comparison in them
- Logos, decorative graphics, flowcharts, or UI mockups (use the design skills instead)
...
### 5. Output
Default to **self-contained HTML/SVG** (single file, inline SVG, no external deps). Use **React (Recharts, or D3-in-React for dot plots/slopegraphs)** when the user is working in a React project. For a quick decision aid, a small inline SVG beside a one-paragraph explanation is often the whole deliverable.
...
{the chart — self-contained HTML/SVG or React component}
